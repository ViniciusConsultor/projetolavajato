...
(select count(servico.cancelado) from servico where  servico.cancelado = 1 and s.servicoid = servico.servicoid) as [O.S. Cancelado],
sum(s.Total) as [Total Vendas], sum(s.Desconto) as [Total Desconto],
...
--select * from servico-- where entrada = '2010-06-10 23:04:43.000'
